...
setup_duckdb <- function() {
  ...
  # Add row names as a column before writing to DuckDB
  mtcars_with_names <- mtcars
  mtcars_with_names$car_name <- rownames(mtcars)
  mtcars_with_names$row_id <- 1:nrow(mtcars)  # Add explicit row_id column

  # Write the modified dataframe to DuckDB
  dbWriteTable(con, "mtcars", mtcars_with_names)
  ...
  layout_sidebar(
    sidebar = sidebar(
      ...
      tags$ul(
        ...
        tags$li("Show a scatter plot labeled with car names")
      )
    ),
...
server <- function(input, output, session) {
  ...
  dataset_summary <- reactive({
    ...
    sum_stats <- capture.output({
      ...
      print(summary(df[, !names(df) %in% c("car_name", "row_id")]))
      cat("\nVariable correlations:\n")
      cor_matrix <- cor(df[, sapply(df, is.numeric) & !names(df) %in% c("car_name", "row_id")])
  ...
  system_prompt <- reactive({
    paste0(
      ...
      dataset_summary(),
      "\n\nThe available variables in mtcars are: mpg, cyl, disp, hp, drat, wt, qsec, vs, am, gear, carb, car_name, and row_id. ",
      ...
      "\n- You can use the car_name column to label points in scatter plots with geom_text() or geom_label()",
  ...
  observeEvent(input$chat_user_input, {
    ...
    # Current dataframe for plotting
    df <- mtcars_data()
      ...
      (function(response) {
        ...
        tryCatch({
          ...
          if (!is.null(expr) && length(expr) > 0) {
            # Create a new environment with data accessible
